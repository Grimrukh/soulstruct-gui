...
class ConnectCollisionCreator(SuperFrame):
    ...
    def __init__(self, collision: MSBCollision, game_maps: tuple[Map], master=None):
        super().__init__(master=master, toplevel=True, window_title="Create Connect Collision")
        ...
        with self.set_master(padx=20, pady=20, auto_rows=0, grid_defaults={"pady": 5}):
            ...
            self.Button(text="Create Connect Collision", command=self.done, width=30)
    ...
    def done(self, confirm=True):
        if confirm:
            try:
                ...
                self.error_dialog("Connect Collision Error", f"Could not create Connect Collision. Error:\n{ex}")
                return
        self.quit()
    ...
    def build_entry_context_menu(self):
        ...
        if msb_type == "Parts" and msb_subtype == "Collisions":
            self.context_menu.add_command(
                label="Create Connect Collision",
