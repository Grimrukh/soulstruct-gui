...
class _ModuleFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if "\\soulstruct_gui\\" in record.pathname:
            record.modulepath = "soulstruct_gui." + (
                 record.pathname.split("\\soulstruct_gui\\", 1)[-1].replace("\\", ".").removesuffix(".py")
            )
        else:
...
LOG_PATH = str(Path(_path_source).parent / "soulstruct_gui.log")
