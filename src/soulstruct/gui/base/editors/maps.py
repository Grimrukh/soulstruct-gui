...
from soulstruct.base.maps.msb.utils import BitSet, BitSet128, BitSet256, BitSet1024
...
from soulstruct.gui.base.utilities import (
    bind_events,
    NameSelectionBox,
...
from soulstruct.gui.base.editors.base_editor import EntryRow
from soulstruct.gui.base.editors.field_editor import BaseFieldEditor, FieldRow
...
if tp.TYPE_CHECKING:
    ...
    from soulstruct.gui.base.links import MapsLink
    from soulstruct.gui.typing import *
...
class MapFieldRow(FieldRow):
    ...
    def _update_field_BitSet128(self, value: BitSet128):
        """Update `set` field with a sorted list."""
        self.value_label.var.set(repr(value.to_sorted_bit_list()))
        self._activate_value_widget(self.value_label)

    def _update_field_BitSet256(self, value: BitSet256):
        """Update `set` field with a sorted list."""
        self.value_label.var.set(repr(value.to_sorted_bit_list()))
        self._activate_value_widget(self.value_label)

    def _update_field_BitSet1024(self, value: BitSet1024):
        """Update `set` field with a sorted list."""
        self.value_label.var.set(repr(value.to_sorted_bit_list()))
        self._activate_value_widget(self.value_label)
    ...
    def build_field_context_menu(self):
        ...
        if issubclass(self.field_type, BitSet):
    ...
    def _set_group_checkbuttons(self):
        current_group_bit_set = self.master.get_selected_field_dict()[self.field_name]  # type: BitSet
        new_bit_set = BitSetEditBox(
            self.master,
    ...
    def _string_to_BitSet128(self, string):
        try:
            enabled_bits_list = ast.literal_eval(string)
        except ValueError:
            raise InvalidFieldValueError(
                f"Could not interpret string as a `BitSet128` for field {self.field_nickname}: {string}"
            )
        try:
            return BitSet128(set(enabled_bits_list))
        except (TypeError, ValueError):
            raise InvalidFieldValueError(
                f"Could not interpret string as a `BitSet128` for field {self.field_nickname}: {string}"
            )

    def _string_to_BitSet256(self, string):
        try:
            enabled_bits_list = ast.literal_eval(string)
        except ValueError:
            raise InvalidFieldValueError(
                f"Could not interpret string as a `BitSet256` for field {self.field_nickname}: {string}"
            )
        try:
            return BitSet256(set(enabled_bits_list))
        except (TypeError, ValueError):
            raise InvalidFieldValueError(
                f"Could not interpret string as a `BitSet256` for field {self.field_nickname}: {string}"
            )

    def _string_to_BitSet1024(self, string):
        try:
            enabled_bits_list = ast.literal_eval(string)
        except ValueError:
            raise InvalidFieldValueError(
                f"Could not interpret string as a `BitSet1024` for field {self.field_nickname}: {string}"
            )
        try:
            return BitSet1024(set(enabled_bits_list))
        except (TypeError, ValueError):
            raise InvalidFieldValueError(
                f"Could not interpret string as a `BitSet1024` for field {self.field_nickname}: {string}"
            )
...
class MapsEditor(BaseFieldEditor, abc.ABC, tp.Generic[MSB_TYPE]):
    ...
    def _get_field_edit_widget(self, row_index):
        """Handles special cases (Vector3, RegionShape, BitSet, MSBEntry) or goes to parent method."""
        ...
        if issubclass(field_type, BitSet):
            field_value: BitSet
    ...
    def get_field_links(self, field_type: type[GameObjectInt], field_value, valid_null_values=None) -> list:
