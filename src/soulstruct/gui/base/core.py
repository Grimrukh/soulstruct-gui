...
if tp.TYPE_CHECKING:
    ...
    from soulstruct.base.ai import LuaBND, AIScriptDirectory
    from soulstruct.base.game_types import GameEnumsManager
    from soulstruct.base.events import EventDirectory
    from soulstruct.base.ezstate import TalkDirectory
    from soulstruct.base.maps import MapStudioDirectory
    from soulstruct.base.params import GameParamBND
    from soulstruct.darksouls1ptde.params.draw_param import DrawParamDirectory  # TODO: Move to PTDE/DSR subclasses.
    from soulstruct.base.text import MSGDirectory
...
_GAME_MODULE_RE = re.compile(r"^soulstruct\.gui\.(\w+?)\..*$")
_VERSION = importlib.metadata.version("soulstruct-gui")
...
class GameDirectoryProject(abc.ABC):
    ...
    def load_existing_project(self, config_path: Path, new_game_root: Path | str = None) -> bool:
        ...
        if last_save_version.split(".")[0] != _VERSION.split(".")[0]:
            ...
            _LOGGER.warning(
                ...
                f"version number to this version of Soulstruct ({_VERSION}). You may need to export your "
