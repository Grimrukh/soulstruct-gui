...
from soulstruct.base.maps.msb import BitSet128
...
from soulstruct.gui.window import SmartFrame
from soulstruct.gui.base.editors.maps import MapsEditor as BaseMapsEditor, MapEntryRow as BaseMapEntryRow
...
class ConnectCollisionCreator(SmartFrame):
    ...
    def _create_connect_collision(self) -> MSBConnectCollision:
        ...
        try:
            ...
            raise ValueError(
                f"Could not interpret Draw/Display Groups string as a `BitSet128`: {string}"
            )
        try:
            groups = BitSet128(set(enabled_bits_list))
